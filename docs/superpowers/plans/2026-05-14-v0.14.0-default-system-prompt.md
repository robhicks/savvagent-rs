...
**Goal:** Bake a dynamic default system prompt into `savvagent-host` so a default-installed Savvagent gives the model accurate framing about its identity, environment, and tool affordances — closing the "I cannot access GitHub" capability-awareness gap.

**Architecture:** New `default_prompt` module in `savvagent-host` builds a per-session prompt from real data (connected tool names, OS/arch, project root, git presence, app version, trusted shell-availability flag). The existing `project::system_prompt` is replaced with a 3-layer `layered_prompt` (default → embedder override → `SAVVAGENT.md`). Tool-server-supplied text never enters the system message; descriptions still flow through the request's typed `tools` field.
...
NEW:
- `crates/savvagent-host/src/default_prompt.rs` — `PromptEnv`, `AppVersion`, `build()`. Pure-ish: one `.git` stat in `probe()`, otherwise no I/O.

MODIFIED:
- `crates/savvagent-host/src/lib.rs` — declare `default_prompt` module; re-export `AppVersion`.
- `crates/savvagent-host/src/config.rs` — add `default_prompt_enabled: bool`, `app_version: Option<String>`, plus `with_default_prompt_disabled()` and `with_app_version()` builders.
- `crates/savvagent-host/src/project.rs` — add `layered_prompt()` next to the existing `system_prompt()`; later (Task 8) remove `system_prompt()`.
- `crates/savvagent-host/src/tools.rs` — add `pub(crate) fn bash_available(&self) -> bool` on `ToolRegistry`.
- `crates/savvagent-host/src/session.rs` — rewire `Host::start` and `Host::with_components` to assemble the layered prompt; add integration tests.
- `crates/savvagent/src/main.rs` — call `.with_app_version(env!("CARGO_PKG_VERSION"))` at both `HostConfig` construction sites (lines 335 and 356).
...
Add this test inside the existing `#[cfg(test)] mod lazy_bash_tests` block in `crates/savvagent-host/src/tools.rs` (near the bottom of the module, after `bash_spawn_key_equality_discriminates_allow_net`):
...
Run: `cargo test -p savvagent-host bash_available_false_when_no_lazy_bash_slot -- --nocapture`
Expected: FAIL with "no method named `bash_available` found".

- [ ] **Step 3: Add the accessor**

Inside `impl ToolRegistry { ... }` in `crates/savvagent-host/src/tools.rs`, add (place it just after `connect(...)` and before `call_with_bash_net_override`):
...
Append to `crates/savvagent-host/src/config.rs` (create a `#[cfg(test)] mod tests { ... }` block at the bottom if one doesn't exist):
...
Run: `cargo test -p savvagent-host config::tests`
...
In `crates/savvagent-host/src/config.rs`, modify the `HostConfig` struct (around line 35–64) to add the two fields. Find the closing `}` of the struct and add immediately before it:
...
```bash
git add crates/savvagent-host/src/config.rs
...
**Files:**
- Modify: `crates/savvagent-host/src/project.rs`

New 3-layer function (default → override → `SAVVAGENT.md` body). The existing `system_prompt` stays in place for now — `Host::start` still calls it. Task 8 swaps the call site and removes the old function.

- [ ] **Step 1: Write the failing tests**

Append these tests inside the existing `#[cfg(test)] mod tests { ... }` block in `crates/savvagent-host/src/project.rs`, before the final closing `}`:
...
Run: `cargo test -p savvagent-host project::tests::layered`
Expected: FAIL on compile — `layered_prompt` doesn't exist yet.

- [ ] **Step 3: Implement `layered_prompt`**

In `crates/savvagent-host/src/project.rs`, add this function immediately after the existing `system_prompt` function (after the closing `}` at line 111 of the current file, before the `#[cfg(test)]` block):
...
```bash
git add crates/savvagent-host/src/project.rs
...
**Files:**
- Create: `crates/savvagent-host/src/default_prompt.rs`
...
Create `crates/savvagent-host/src/default_prompt.rs` with this initial content:
...
In `crates/savvagent-host/src/lib.rs`, add `mod default_prompt;` to the module declarations near line 21 (alphabetical with the rest). Add this re-export to the `pub use` block:
...
Run: `cargo test -p savvagent-host default_prompt::tests`
Expected: PASS, four tests.

- [ ] **Step 4: Run full crate build + tests + clippy**

Run: `cargo build -p savvagent-host && cargo test -p savvagent-host && rustup run stable cargo clippy -p savvagent-host -- -D warnings`
Expected: clean build, all tests pass, clippy silent.

- [ ] **Step 5: Commit**

```bash
git add crates/savvagent-host/src/default_prompt.rs crates/savvagent-host/src/lib.rs
...
In `crates/savvagent-host/src/default_prompt.rs`, add a new private function below the `const CONVENTIONS` block but above `pub fn build`:
...
Run: `rustup run stable cargo clippy -p savvagent-host -- -D warnings`
Expected: clean.

- [ ] **Step 6: Commit**

```bash
git add crates/savvagent-host/src/default_prompt.rs
...
In `crates/savvagent-host/src/default_prompt.rs`, add this `const` near the other affordances strings:
...
In `crates/savvagent-host/src/default_prompt.rs`, add `impl<'a> PromptEnv<'a>` block (place it immediately after the `PromptEnv` struct definition):
...
Run: `cargo build -p savvagent-host && rustup run stable cargo clippy -p savvagent-host -- -D warnings`
Expected: clean.
...
- Modify: `crates/savvagent-host/src/project.rs` (remove the now-unused `system_prompt` function)
...
The existing `policy_tests` module in `crates/savvagent-host/src/session.rs` (starts at line 1115) already imports the right types, has a `ScriptedProvider` template, and a `config_no_tools()` helper. Mirror that pattern.
...
Then add the `StreamEvent` import. Find the existing `use savvagent_protocol::{` line at the top of `mod policy_tests` (line 1121) and update it to:
...
If `tempfile` is not already a `[dev-dependencies]` entry on `savvagent-host`, check `crates/savvagent-host/Cargo.toml`. The existing `project.rs` tests use `tempfile::tempdir()` so the dev-dep is already there — no Cargo.toml change required.

- [ ] **Step 2: Run tests to verify they fail**

Run: `cargo test -p savvagent-host host_start_default`
Expected: FAIL — three tests fail (current `Host::with_components` still calls `project::system_prompt` which produces `None` for an empty project root; the assertions about `# Savvagent default prompt` won't match).

- [ ] **Step 3: Rewire `Host::start` and `Host::with_components`**

In `crates/savvagent-host/src/session.rs`, find these lines (around 262–263 and 299–300):
...
```rust
        ...
        let savvagent_md_body =
            project::parse_savvagent_md(&config.project_root).body;
...
In `crates/savvagent-host/src/project.rs`, delete the entire `pub fn system_prompt(...)` function (currently at lines 99–111) and its associated tests:
...
(The remaining tests — `front_matter_permissions_round_trip`, `malformed_front_matter_falls_back_silently`, `no_front_matter_treats_whole_file_as_body`, `front_matter_only_no_body` — exercise `parse_text` / `parse_savvagent_md` and stay.)
...
```bash
git add crates/savvagent-host/src/session.rs crates/savvagent-host/src/project.rs
...
The TUI passes its own `CARGO_PKG_VERSION` so end users see "Savvagent version: 0.14.0" (the App variant), not the "host crate version" fallback.

- [ ] **Step 1: Read the two construction sites**

Run: `grep -n "HostConfig::new" crates/savvagent/src/main.rs`
Expected output: two lines around 335 and 356.

- [ ] **Step 2: Add `.with_app_version(env!("CARGO_PKG_VERSION"))` at each site**

In `crates/savvagent/src/main.rs`, find this block (around line 334–342):
...
Run: `cargo build -p savvagent && cargo test --workspace`
Expected: clean build, all tests pass.

- [ ] **Step 4: Smoke-test the running TUI (manual)**

Run: `cargo run -p savvagent -- --help`
...
- `HostConfig::with_app_version` lets embedders pass the binary's
  ...
  installed version (not the `savvagent-host` crate version).
...
(If `README.md` has no "Configuration" section, search for the closest equivalent section — "Running Savvagent", "Usage", or similar — and add this subsection there.)
