//! MCP server wrapper around [`OpenAiProvider`].
//!
...
/// MCP server that exposes [`OpenAiProvider`] as an SPP-conformant
/// `complete` tool. Multiple in-flight calls are safe; the underlying provider
/// is shared via `Arc`.
#[derive(Clone)]
pub struct OpenAiMcpServer {
    provider: Arc<OpenAiProvider>,
    #[allow(dead_code)]
    tool_router: ToolRouter<Self>,
}

impl std::fmt::Debug for OpenAiMcpServer {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("OpenAiMcpServer").finish_non_exhaustive()
    }
}
...
impl OpenAiMcpServer {
    ...
    pub fn from_shared(provider: Arc<OpenAiProvider>) -> Self {
        Self {
            provider,
            tool_router: Self::tool_router(),
        }
...
#[tool_handler]
impl ServerHandler for OpenAiMcpServer {
