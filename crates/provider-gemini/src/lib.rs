...
//! Crate layout:
//!
//! - [`api`] — typed mirror of the relevant subset of Gemini's
...
//!
//! The eventual binary wraps a `GeminiProvider` in an `rmcp` Streamable
...
/// API version path segment Gemini's REST surface lives under.
...
impl GeminiProvider {
    /// Start configuring a [`GeminiProvider`].
...
impl GeminiProviderBuilder {
    /// Set the API key. If unset, [`build`](Self::build) reads
    /// `GEMINI_API_KEY` (preferred) or `GOOGLE_API_KEY` from the
    /// environment.
    ...
    pub fn build(self) -> Result<GeminiProvider, BuildError> {
        let api_key = self
            .api_key
...
/// [`GeminiProviderBuilder::build`] failures.
#[derive(Debug, thiserror::Error)]
pub enum BuildError {
    /// Neither the builder nor `GEMINI_API_KEY` / `GOOGLE_API_KEY` provided
    /// an API key.
    #[error("GEMINI_API_KEY (or GOOGLE_API_KEY) is not set and no api_key was provided")]
    MissingApiKey,
...
impl ProviderHandler for GeminiProvider {
    async fn complete(
        ...
        let body = translate::request_to_gemini(&req);
        ...
        let model = url_safe_model(&req.model);
        let mut url = format!("{}/{API_VERSION}/models/{model}:{action}", self.base_url);
        if want_stream {
            url.push_str("?alt=sse");
        }

        tracing::trace!(%url, want_stream, "POSTing to Gemini");
        let resp = self
            .http
            .post(&url)
        ...
        tracing::trace!(status = %resp.status(), "Gemini responded");
        ...
        if want_stream {
            ...
            let raw: api::GenerateContentResponse =
            ...
            Ok(translate::response_from_gemini(raw))
        }
    }
}

/// Gemini model names are URL-safe by convention (`gemini-2.5-flash`), but
/// users sometimes paste `models/...` prefixes. Normalize here.
fn url_safe_model(model: &str) -> &str {
...
/// Build the `axum::Router` that serves [`GeminiMcpServer`] over MCP
...
pub fn router(provider: Arc<GeminiProvider>) -> axum::Router {
